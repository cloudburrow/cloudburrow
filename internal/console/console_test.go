...
//
// A Go interface is satisfied by a type, so one provider type shared by
// several screens advertised Detail for all of them the moment any one could
// serve it. That shipped: every Kubernetes screen reported detail:true while
// only Pods had a detail function, so clicking a Service produced a link to
// "rows cannot be opened" — a working-looking control offered by the
