...
**Backing component:** CloudBurrow's own server (`internal/service/storage`, #485), built to
Google's documentation and the storage v1 discovery document, which it embeds: every method of
that document is either served or answers **501 `notImplemented` naming it**. It is the only
...
or real-tool compat test annotated for it passes, **Implemented** when it is served but no such
test covers it, and **Unimplemented** when it answers 501. `TestCoverageStorageMatchesCompatibility`
fails if a row below disagrees with that report. The official Go client retries every 5xx, 501
included, until its deadline, so an unimplemented method costs a client its timeout rather than
failing at once.
...
|---|---|---|
| `buckets.insert`, `buckets.get`, `buckets.delete` | **Verified** | `TestStorageBucketLifecycle`. A non-empty bucket delete is 409 (`TestStorageNonEmptyBucketDelete`; UNVERIFIED for the JSON API). |
| `buckets.list` | **Verified** | `TestStorageBucketsList`; buckets are scoped by project. |
| `buckets.patch` | **Verified** | `TestStorageBucketPatchKeepsOmittedFields`, `TestBucketUpdateChangesMetadata`, `TestStorageBucketMetagenerationPreconditions`: a JSON merge patch, with omitted fields kept (#374). |
| `buckets.update` | **Implemented** | A full replace. No official client sends it: the Go client's `Update` is a patch. |
| `buckets.lockRetentionPolicy` | **Verified** | `TestStorageLockRetentionPolicy`. |
| `buckets.restore` | **Implemented** | Raw HTTP in `TestStorageBucketDeleteIgnoresSoftDeleted`; no official-SDK test calls it. |
| `buckets.getStorageLayout` | **Verified** | `TestGcloudStorageParallelCompositeUpload`: gcloud reads it before a parallel composite upload. |
| `buckets.getIamPolicy`, `buckets.setIamPolicy`, `buckets.testIamPermissions` | **Verified**, *stored, not enforced* | `TestStorageBucketIAMRoundTrip`, `TestStorageBucketIAMStaleEtag`, under [ADR-0006](adr/0006-iam-policy-surface.md) as amended by #504: a policy round-trips with its etag and version, conditions are refused, and nothing is enforced. |
| `projects.hmacKeys.create`, `projects.hmacKeys.get`, `projects.hmacKeys.update`, `projects.hmacKeys.delete`, `projects.hmacKeys.list`, `projects.serviceAccount.get` | **Verified** | `TestStorageHMACKeyLifecycle`, `TestStorageHMACKeyLimit`, `TestGcloudStorageHMAC` (#505). A key's secret is shown once, at creation. |
| `notifications.insert`, `notifications.get`, `notifications.list`, `notifications.delete` | **Verified** | `TestNotificationConfigsCRUDThroughTheSDK`; see the notifications section. |
| `managedFolders.list` | **Verified** | Empty (`TestGcloudStorageClearLabels`, through `gcloud storage rm -r`): `managedFolders.insert` is 501, so none can exist. |
| `objects.insert`, `objects.get`, `objects.delete` | **Verified** | `TestStorageObjectRoundTrip`. |
| `objects.list` | **Verified** | `TestStorageListWithPrefix`: prefix **and** delimiter, with the synthetic `dir/sub/` prefix. |
| `objects.patch` | **Verified** | `TestObjectMetadataUpdate`; the bytes are re-read afterwards. |
| `objects.update` | **Implemented** | A full replace. No official-SDK test sends it. |
| `objects.rewrite` | **Verified** | `TestObjectCopyAndRewrite`, within a bucket and across buckets. |
| `objects.compose` | **Verified** | `TestStorageCompose`, `TestGcloudStorageParallelCompositeUpload`. |
| `objects.copy`, `objects.move` | **Implemented** | Served; the Go client's copier uses `objects.rewrite`, and no official-SDK test calls either (#495). |
| `objects.restore` | **Verified** | `TestStorageSoftDeleteAndRestore`, `TestStorageRestoreNotSoftDeleted412`, `TestStorageRestoreWithoutPolicy412`. |
| `objects.bulkRestore`, `objects.watchAll`, `channels.stop`, bucket and object ACLs, object and managed-folder IAM, the other `managedFolders` methods, `folders`, `anywhereCaches` and `operations` | **Unimplemented** | 501 naming the method (`TestStorageACLNotImplemented`), never an empty policy or list. |
...
| Multipart upload (`uploadType=multipart`) | **Verified** | `TestMultipartUploadStoresMetadataWithContent`: custom metadata and content type survive alongside the bytes. |
| Resumable upload (`uploadType=resumable`) | **Verified** | `TestStorageResumableUploadAndRangedRead` (8 MiB in 256 KiB chunks) and `TestResumableUploadCompletes`; the resume after an interrupted chunk is `test_resumable_upload_resumes_after_interruption` (#516). |
| Download and ranged download | **Verified** | `TestStorageObjectRoundTrip`; `TestStorageResumableUploadAndRangedRead`, byte-exact; `test_if_generation_match_zero`. |
| Generation and metageneration preconditions | **Verified** | `TestStorageGenerationPreconditions`: `DoesNotExist` on an existing object and a stale `GenerationMatch` are both 412, and a matched precondition advances the generation. |
...
Built to Google's spec in steps (#485), one issue per feature:

#### Buckets (#490)

Are served: insert, get, list (prefix, pages of at most 1000), patch as a JSON merge patch (an omitted field stays, `null` clears it, a `null` label removes that key), update, delete, and metageneration with `ifMetagenerationMatch`/`NotMatch` (412 `conditionNotMet`; 304 for a read). Every settable field is kept, stored or refused by name with 400, never dropped with 200; see the bucket fields table below. A non-empty bucket delete is 409 (UNVERIFIED for the JSON API).

#### Objects (#491)

Media and multipart uploads (`uploadType=media`, `multipart`; a metadata-only POST is 400 `wrongUrlForUpload`), with `Content-MD5`, `x-goog-hash` and the metadata's `md5Hash`/`crc32c` verified and a mismatch refused; `ifGenerationMatch` (including 0, "only if absent") and the other three preconditions; metadata reads, `alt=media`, `/download/storage/v1/…`, and XML `GET`/`HEAD /{bucket}/{object}` with the `x-goog-*` headers; one byte range (`a-b`, `a-`, `-N`; 206, or 416 with `Content-Range: bytes */size`); and a new generation, metageneration 1, on every finalize. `TestStorageObjectRoundTrip`, `TestStorageChecksums` and `TestMultipartUploadStoresMetadataWithContent` pass against it.

#### Object metadata (#492)

Patch (merge; a `null` metadata key removes it) and update (replace), each advancing the metageneration and keeping the generation and bytes; `customTime` only moves forward and cannot be removed, as documented; A body may restate the object's `storageClass` but not change it, which only a rewrite does (#515); holds and retention follow the retention rules below. Preconditions combine with AND on the live generation unless one is named: `ifGeneration(Not)Match`, `ifMetageneration(Not)Match` (412, or 304 for a failed NotMatch on a read), `If-Match` (412) and `If-None-Match` (304 on reads), and for the XML API `x-goog-if-generation-match`, `x-goog-if-metageneration-match`, `If-Modified-Since` (304) and `If-Unmodified-Since` (412). `TestStorageGenerationPreconditions` and `TestObjectMetadataUpdate` pass against it.

#### Listing (#494)

`prefix`, `delimiter` with synthetic `prefixes` (which count toward `maxResults`), `includeTrailingDelimiter`, `startOffset`/`endOffset`, `matchGlob` (`**`, `*`, `?`, `[…]`, `[!…]`, `{a,b}`), and pages of at most 1000 in byte order. `TestStorageListWithPrefix` passes against it.

#### Resumable uploads (#493)

A session from `uploadType=resumable` (200 with `Location`; the `upload_id` is random), chunks by PUT or POST with `Content-Range: bytes a-b/TOTAL|*` (every chunk but the last a multiple of 256 KiB; persisted bytes immutable, so a re-sent range is skipped), and an incomplete chunk answered **308 with `Range: bytes=0-N`**, or, with `X-GUploader-No-308: yes` as the Go client sends it, **200 with `X-Http-Status-Code-Override: 308`**. Both handshakes are undocumented and required by the clients. `bytes */TOTAL` and `bytes */*` query the status; the final chunk verifies `x-goog-hash` and applies `X-Goog-Meta-*`; DELETE cancels with 499. A session lasts a week, then answers 410 for a week, then 404. Sessions and their chunks survive a restart in persistent mode. `TestStorageResumableUploadAndRangedRead` and `TestResumableUploadCompletes` pass against it.

#### Compose, copy, rewrite and move (#495)

Compose takes 1 to 32 sources of one storage class, with per-source `generation` and `ifGenerationMatch` and `deleteSourceObjects`; the result has **no MD5**, a CRC32C combined from the components, and `componentCount` (counting a composite's components). Copy and rewrite share the source's bytes, cross buckets and honour `ifSource*`; rewrite changes the storage class, and with `maxBytesRewrittenPerCall` (a multiple of 1 MiB) proceeds by tokens, each good for a week and once. Move renames atomically within a bucket. `kmsKeyName`, `dropContextGroups` and the destination ACL and key parameters are refused by name. `TestStorageCompose`, `TestStorageCopy` and `TestObjectCopyAndRewrite` pass against it; the Go client's copier sends `objects.rewrite`, so `objects.copy` and `objects.move` are exercised only in process and by raw HTTP, and the coverage report marks them Implemented rather than Verified.

#### Batch (#496)

`POST /batch/storage/v1` takes a `multipart/mixed` body of `application/http` parts, dispatches each through the same handler as the direct call, and answers each in order with `Content-ID: <response-…>`; an upload or a download inside a batch is refused per part, and more than 100 calls refuse the whole batch (the refusal is UNVERIFIED). A part's body needs no `Content-Length`: the Python client sends none, and the body is the rest of the part.

#### Object versioning (#498)

On a bucket with `versioning.enabled`, replacing or deleting the live version keeps it as a **noncurrent** version with its generation and a `timeDeleted`; noncurrent versions appear only with `versions=true` (each name's oldest first, paged within a name) or when named by `generation=`, and can be read, patched and used as a copy source. A delete that names a generation removes that version permanently. Disabling versioning stops new noncurrent versions and keeps the existing ones, `ifGenerationMatch=0` succeeds when only noncurrent versions exist, and a bucket holding only noncurrent versions is not empty (409). Versions survive a restart in persistent mode. `TestStorageVersioningKeepsNoncurrent`, `TestStorageVersioningDeleteKeepsNoncurrent`, `TestStorageVersioningDisableKeepsExisting`, `TestStorageDoesNotExistWithOnlyNoncurrent` and the restart probe `TestStorageVersioningPersistentMode` pass against it.

#### Soft delete (#499)

Every bucket has a soft delete policy, 7 days unless set (7 to 90 days, or 0 for none; `effectiveTime` moves when the retention changes). Under it, a version that leaves the bucket (deleted, overwritten, or a deleted noncurrent version) is kept **soft-deleted** with `softDeleteTime` and `hardDeleteTime`: hidden from reads and listings, listed with `softDeleted=true`, its metadata readable by generation (never its bytes), and `objects.restore` makes it live again as a new generation with metageneration 1, retiring any live version it replaces. A restore answers **412 `softDeletePolicyNotSet`** on a bucket without a policy and **412 `objectNotSoftDeleted`** for a live or noncurrent generation, as the status-codes page lists. A bucket whose only objects are soft-deleted can be deleted; it is then soft-deleted itself under its own policy, readable and listable with `softDeleted=true`, and `buckets.restore` brings it back empty, with its soft-deleted objects restorable again (a live bucket of the same name refuses the restore with 409, UNVERIFIED). A policy change applies only to later deletions. Reads treat anything past its `hardDeleteTime` as gone, and a sweep removes it. A move is a rename and leaves nothing soft-deleted (UNVERIFIED). `bulkRestore` is 501. `TestStorageSoftDeleteAndRestore`, `TestStorageRestoreNotSoftDeleted412`, `TestStorageRestoreWithoutPolicy412` and `TestStorageBucketDeleteIgnoresSoftDeleted` pass against it.

#### Retention and holds (#500)

With the status-codes page's reasons: a bucket `retentionPolicy` (1 s to 3,155,760,000 s) applies to existing and new objects, which report `retentionExpirationTime`; an object younger than the period cannot be deleted or replaced (**403 `retentionPolicyNotMet`**), though a versioned bucket may make it noncurrent, and its metadata stays editable. `lockRetentionPolicy` needs `ifMetagenerationMatch` (400 without, 412 stale) and a policy (400 `badRequest`); a locked period can grow but not shrink or be removed (400 `badRequestException`; the discovery document says PERMISSION_DENIED, the status-codes page 400). `temporaryHold` and `eventBasedHold` block delete and replace (**403 `objectUnderActiveHold`**) but not metadata changes; releasing an event-based hold restarts the retention clock, and no `retentionExpirationTime` is given while it is held; `defaultEventBasedHold` holds each new object. Object retention `{mode, retainUntilTime}` needs a bucket created with `enableObjectRetention=true` (400 `invalid` otherwise, and with an event-based hold); removing, shortening or locking an Unlocked one needs `overrideUnlockedRetention=true` (403 `forbidden`), and a Locked one only extends. A copy, rewrite or restore does not inherit holds or retention. Holds also block a live version becoming noncurrent (UNVERIFIED). `TestStorageRetentionBlocksDelete`, `TestStorageLockRetentionPolicy`, `TestStorageHoldsBlockDelete` and `TestStorageObjectRetentionRequiresEnabledBucket` pass against it.

#### Lifecycle (#501)

`lifecycle.rule` is validated when set, and an unknown action or condition is 400 naming it, with the old configuration kept. Actions are Delete, SetStorageClass and AbortIncompleteMultipartUpload (#508; with only `age`, `matchesPrefix` and `matchesSuffix`, as documented). Conditions are `age`, `createdBefore`, `customTimeBefore`, `daysSinceCustomTime`, `daysSinceNoncurrentTime`, `isLive`, `matchesPrefix`/`matchesSuffix` (at most 1000 across all rules), `matchesStorageClass`, `noncurrentTimeBefore`, `numNewerVersions` and `sizeAboveBytes`/`sizeBelowBytes` (at most 5 TiB). The rules run at least hourly on the server's clock, and on demand with `POST /_cloudburrow/lifecycle`. A rule's conditions combine with AND; `age` 0 is met at midnight UTC after creation, and age N N days after it. Delete takes precedence over SetStorageClass, and the cheapest target class wins; a class only moves to a cheaper one. A hold or an unmet retention period keeps Delete from taking effect, and a deleted live version becomes noncurrent or soft-deleted as any delete does. No limit on the number of rules is imposed, since none is documented. `TestStorageLifecycleConfigRoundTrip` and `TestStorageLifecycleUnknownCondition400` pass against it.

#### CORS (#502)

The bucket's `cors` (`origin`, with `*`; `method`; `responseHeader`; `maxAgeSeconds`, 3600 by default) is kept and validated. **XML endpoints** follow it: a preflight is answered from the first rule that matches its origin, its method and every requested header, with that rule's methods and max age; a preflight that matches none gets **200 with no CORS headers**, as documented; an actual request that matches gets `Access-Control-Allow-Origin` and the rule's `responseHeader` as `Access-Control-Expose-Headers`. **JSON endpoints** (`/storage/v1`, uploads, downloads, batch) always allow, whatever the bucket says: the Origin echoed, `DELETE, GET, HEAD, PATCH, POST, PUT`, a Max-Age of 3600, the requested headers echoed, and `Access-Control-Allow-Credentials: true` on simple requests (the last two UNVERIFIED). A resumable session URI answers with the Origin that started the session. `TestStorageCORSRoundTrip`, `TestStorageXMLPreflightMatches`, `TestStorageXMLPreflightMismatch200NoHeaders` and `TestStorageJSONAlwaysAllowsCORS` pass against it.

#### XML API subset (#507)

Path-style or virtual-hosted (`{bucket}.{host}` with `storage-server --host`): object `PUT` (with `x-goog-meta-*`, `Cache-Control` and the other content headers, `x-goog-storage-class`, `x-goog-if-generation-match`/`-metageneration-match` and `Content-MD5`; 200 with the ETag, `x-goog-generation` and `x-goog-hash`, no body), `GET`/`HEAD` (#491) and `DELETE` (204); bucket `GET` (a `ListBucketResult` with `prefix`, `delimiter` and its `CommonPrefixes`, `marker`, `max-keys`, `IsTruncated` and `NextMarker`) and `DELETE`; and resumable uploads, where `POST` with `x-goog-resumable: start` is **201 with the session URI**, chunks are `PUT` to it with `Content-Range` (308 while incomplete), and a `DELETE` cancels with **204**. Errors are `<Error>` documents: `NoSuchBucket`, `NoSuchKey`, `PreconditionFailed`, `BucketNotEmpty` (409), `InvalidArgument`, `InvalidRange`, `AccessDenied`. `x-goog-copy-source`, `x-goog-acl` and customer encryption headers are refused by name; bucket `PUT`, service `GET` and form `POST` are 501. Signed URLs (#509) are verified; see the data plane. `TestStorageXMLPutGetDelete`, `TestStorageXMLListBucket`, `TestStorageXMLResumable201And204`, `TestStorageXMLErrorCodes` and `TestStorageXMLHostStyle` pass against it.

#### XML multipart uploads (#508)

`POST ?uploads` (an `InitiateMultipartUploadResult` in the S3 namespace the clients parse), `PUT ?partNumber=N&uploadId=` for N 1 to 10,000 (the ETag, and `x-goog-hash` the Python client validates; the same number again replaces the part), `POST ?uploadId=` with `CompleteMultipartUpload` (parts in ascending order with their ETags; every part but the last at least 5 MiB; **no MD5** on the object, as documented, and a CRC32C), `DELETE ?uploadId=` (**204**), list parts (`max-parts`, `part-number-marker`, at most 1000) and `GET /{bucket}?uploads`. **Preconditions are refused with 400 `NotImplemented`**, as the docs say they are not supported. The codes for a part out of order, a bad ETag or a small part are S3's `InvalidPartOrder`, `InvalidPart` and `EntityTooSmall`, and the completed upload's ETag is S3's `"md5-of-md5s-N"`; all UNVERIFIED. An incomplete upload stays until aborted, or until a lifecycle `AbortIncompleteMultipartUpload` rule (age, `matchesPrefix`, `matchesSuffix`) removes it. Google's Python `transfer_manager.upload_chunks_concurrently` uploads a 20 MiB file in 5 MiB parts against it: `test_transfer_manager_xml_multipart_upload` runs in CI (#516). `TestStorageXMLMultipartAbort204`, `TestStorageXMLMultipartListParts` and `TestStorageXMLMultipartPreconditionRefused` pass against it.

#### Persistence by mode (#512; the #481 lesson)

With `--data-dir`, `storage-server --mode persistent` (the default) keeps buckets, every object version's bytes and metadata, and in-progress resumable sessions across a restart. `--mode ephemeral` deletes the store an earlier run left in that directory before serving, so nothing survives whatever is on disk: the choice is made by the mode, never by whether a store exists. CI's restart probe `TestStorageAcrossRestart` runs at setup, after a persistent restart (a bucket, a two-version object and a 256 KiB-in session: **present**) and after an ephemeral restart on the same directory (**absent**); unit `TestStorageEphemeralClearsLeftoverStore` does the same in-process. `up` passes the instance's mode to the in-cluster server (#514): measured with a real instance, an object written in persistent mode read back after `stop` and `up`, and after `up --mode ephemeral` the bucket was gone and the Deployment had no volume.

#### In the cluster (#514)

`up` runs **one** Deployment for the host and the cluster, with no notification front, no second Deployment and no Host-binding workaround (#268), because the server builds `mediaLink` from each request's Host. Its image is built at `up` from the Linux `cloudburrow-storage` binary embedded in the CLI, on a distroless base pinned by digest (`build.storageServerBase`), and `kind load`ed; nothing is published or downloaded but the base. Readiness is a real request (`GET /storage/v1/b?project=_`). A NetworkPolicy allows egress only to DNS and the Pub/Sub emulator; kind's default CNI does not enforce NetworkPolicy, so `TestStorageNoEgress` proves in-process that a whole workload resolves no name at all. Measured against a real instance: 58 builtin storage and notification compat tests passed through the tunnel, the e2e `TestAcceptanceWorkflow` passed with the worker's `STORAGE_ENDPOINT` on the single Deployment, and the CLI scrapes the server's calls into `/admin/events` and `/metrics`. Measured for #514: a pod could **not** reach the CLI's loopback-bound ports through the kind gateway, but **could** through `host.docker.internal` on Docker Desktop, admin API included (#553). **The official Go client retries every 5xx, 501 included, until its deadline**, so a method not built yet costs a client its timeout rather than failing at once. CI runs the whole compat suite against it twice: through the tunnel on an instance (the `official SDK compatibility (storage)` shard, #565), and against a standalone `cloudburrow storage-server` with the Python suite, the restart probes and a registered signing certificate (`check` job).

#### Official clients and tools (#516, #517)

**The official Python client** (google-cloud-storage 3.14.1, #516) runs against the storage server in CI: CRUD with a resumable upload, a resume after an interrupted chunk (`recover()`'s `bytes */*` answered 308 with the `Range` held), `mediaLink` on the emulator's host, `if_generation_match=0` and a ranged download, batch, `transfer_manager` XML multipart, and a V4 signed URL. **gcloud storage** (586.0.0, #517), configured by `gcloud-setup` alone, with every non-loopback request refused by a test proxy: `storage ls`, `buckets create`, `buckets update --update-labels` then `--clear-labels`, `objects update`, `hmac create/list/update/delete`, and `cp` of 200 MiB by the parallel composite path. That path reads `buckets.getStorageLayout` and composes with `deleteSourceObjects` in the request body, which deletes the components, and a download over the sliced threshold reads it back in four ranged slices. `gcloud storage rm -r` lists `managedFolders`, which is empty since none can be created (every other managedFolders method is 501). On macOS, gcloud's `gcloud-crc32c` helper under Gatekeeper quarantine hangs on any file, CloudBurrow or not; the test checks the sliced download by SHA-256 instead. **gsutil** (5.37) requires HTTPS on both APIs (`gcs_json_api.py` hard-codes `https://`; `is_secure = False` is refused), and CloudBurrow serves plain HTTP, so it works only behind a TLS front; `TestGsutilJSONAndHMACXML` supplies one. Through it, cp, ls and rm work over JSON with no credentials and over XML with HMAC-only credentials. gsutil cannot download over XML from any port but 443 or 80, whatever the server (its `_AddCustomEndpointToKey` sets the port as a string, and boto formats it with `%d`). apitools sent **no** `X-GUploader-No-308` (0 of 15 requests).

### Bucket fields

Every settable Bucket property of the discovery document (#503). *Kept* fields are stored, echoed and act as documented. *Stored, no effect* fields are stored and echoed as sent, but nothing here acts on them. *Refused* fields get 400 naming the field, never 200 and a silent drop. `TestStorageBucketFieldsNeverSilentlyDropped` checks this list against the schema.

| Field | Behaviour | Issue |
|---|---|---|
| `location`, `storageClass`, `labels` (a `null` key removes it) | Kept; the location is fixed at creation | #490, #503 |
| `versioning` | Kept | #498 |
| `softDeletePolicy` | Kept | #499 |
| `retentionPolicy`, `defaultEventBasedHold`, `objectRetention` (with `enableObjectRetention=true`) | Kept | #500 |
| `lifecycle` | Kept | #501 |
| `cors` | Kept | #502 |
| `website` | Stored, no effect: no website serving | #503 |
| `logging` | Stored, no effect: no usage logs are written | #503 |
| `encryption` (`defaultKmsKeyName`) | Stored, no effect: objects are not encrypted with the key | #503 |
| `billing` (`requesterPays`) | Stored, no effect: nothing is billed, and `userProject` is accepted and ignored | #503 |
| `iamConfiguration` (`uniformBucketLevelAccess`, `publicAccessPrevention`) | Stored, no effect: there are no ACLs, so access is uniform anyway | #503 |
| `rpo` (`DEFAULT` or `ASYNC_TURBO`) | Stored, no effect: there is no replication | #503 |
| `autoclass` | Stored, no effect: classes move only by rewrite or lifecycle | #503 |
| `customPlacementConfig` | Stored, no effect: there is one location | #503 |
| `hierarchicalNamespace` | Stored, no effect: no folders API; names stay flat | #503 |
| `ipFilter` | Refused, 400 naming it | — |
| `acl`, `defaultObjectAcl` | Refused, 400 naming them: ACL methods are not implemented | — |

### UNVERIFIED choices

Where no Google page states a behaviour, the server makes a choice and marks it UNVERIFIED in the
test that pins it; the method-level codes are also listed in [docs/coverage/storage.md](coverage/storage.md).

- `buckets.delete` of a non-empty bucket answers 409, as the XML API documents `BucketNotEmpty`; no page states the JSON API's status.
- `buckets.setIamPolicy` with a stale etag answers 412, the JSON API's precondition code.
- A live bucket of the same name refuses `buckets.restore` with 409.
- A move is a rename and leaves nothing soft-deleted; holds block a live version becoming noncurrent.
- More than 100 calls refuse a whole batch.
- The JSON API's CORS defaults beyond `Allow-Origin`, `Allow-Methods` and `Max-Age`: the requested headers echoed, and `Allow-Credentials` on simple requests.
- XML multipart part order, ETag and size errors use S3's `InvalidPartOrder`, `InvalidPart` and `EntityTooSmall`, and the completed upload's ETag is S3's `"md5-of-md5s-N"`.
- A signed URL's `X-Goog-Expires` is at most 604800 seconds, from the docs rather than a measured refusal.
...
| Cloud Storage | **Verified** | `TestStateRestoresStorage` and `TestStateRestoresVersionsAndHolds`. The state is exported from the storage server's store directly (#511, `builtinStorageSnapshotter`), so a restore fires no notifications: every bucket with all its fields, soft-deleted buckets, every object version (live, noncurrent and soft-deleted) with its generation, metageneration, holds, retention and customTime, notification configurations and undelivered events, IAM policies, sessions and multipart uploads, and the bytes, streamed. A load verifies that every blob hashes to its content address and that every object version's CRC32C matches, before it replaces anything, in one transaction. **HMAC key secrets are excluded**, and the archive says so (`secretsExcluded`): a restored key keeps its metadata and cannot sign. `TestSnapshotRoundTripKeepsVersionsAndHolds`, `TestSnapshotExcludesHMACSecrets`, `TestSnapshotImportVerifiesBytes`, `TestBuiltinStorageSnapshotRoundTrip` (10 MiB, streamed, never held whole in memory). |
...
| Terraform `google` provider create / read / destroy | **Verified** | `TestTerraformAppliesAndDestroysThroughTheWrapper`: `google_storage_bucket`, `google_pubsub_topic`, `google_secret_manager_secret`, `google_secret_manager_secret_iam_member` (#365), `google_cloud_tasks_queue` and `google_cloud_tasks_queue_iam_member` (#366; IAM stored, not enforced) applied through `cloudburrow terraform` with `hashicorp/google` ~> 8.0 on Terraform 1.16.4, read back through the official SDKs, destroyed, and confirmed gone. A second `plan` after the apply is clean for the Secret Manager and Cloud Tasks resources. **A bucket plans clean too** (#515), with no `-target`. (Before the cut-over, #519, fake-gcs-server forced a replacement by reporting `location` as `US-CENTRAL1` for a bucket created in `US`, measured on #515; that was #374.) Also verified, apply, clean plan, and destroy: `TestTerraformStorageResources` covers a bucket with versioning, labels, a lifecycle rule, CORS, `retention_policy` and `soft_delete_policy`; a `google_storage_bucket_object` with `temporary_hold` and `event_based_hold` on a bucket with `enable_object_retention` (a held object refuses deletion; releasing the holds is an update that restates the object's `storageClass`, which is accepted when unchanged); `google_storage_bucket_iam_member` (stored, not enforced); `google_storage_hmac_key`; and `google_storage_notification` to a `google_pubsub_topic`. `TestTerraformForceDestroyVersionedBucket` shows `force_destroy` removing a bucket with noncurrent versions. The wrapper sets `storage_custom_endpoint`, `pubsub_custom_endpoint`, `secret_manager_custom_endpoint`, `cloud_tasks_custom_endpoint`, the Resource Manager and Billing endpoints, `project` and a fixture `access_token`. `terraform init` still downloads the provider once. |
| Terraform, other services' endpoints | **Not set** | `cloudburrow terraform` sets an endpoint only for a service with a Verified row here, and names the enabled services it left out: their resources would otherwise be sent to real Google. **Cloud KMS is not wired** (#388): only `:encrypt` and `:decrypt` are served over JSON, so the provider's key ring and key calls cannot be answered; `kms_custom_endpoint` is not set and kms is named as left out (`TestTerraformLeavesOutKMS`). |
| Terraform resources beyond Storage, Pub/Sub, Secret Manager and Cloud Tasks queues | Planned | Untested is untested. |
...
| IAM policies on key rings and keys | **Unimplemented** | Tested (`TestKMSOtherServicesAreUnimplemented`, #397): GetIamPolicy, SetIamPolicy and TestIamPermissions on a key ring and a crypto key are UNIMPLEMENTED, never an empty policy. [ADR-0006](adr/0006-iam-policy-surface.md) does not extend policy storage to Cloud KMS. |
...
| Durability | **Verified, measured** | CI's restart probe (#418): `TestKMSRestartSetup` runs alone just before `stop` and leaves a key with an ENABLED primary v1, a DISABLED v2, a DESTROY_SCHEDULED v3 and a v1 ciphertext. After `stop` and `up` in persistent mode, `TestKMSAcrossRestart` decrypts the ciphertext to the original plaintext and finds the states, `destroy_time`, the primary and the next version number (4) unchanged. **Ephemeral mode is not honoured: measured, the ring survives `up --mode ephemeral`** (#481), because KMS keeps its Secrets in the managed namespace whatever `--mode` says, and ephemeral mode keeps the namespace. The probe asserts what was measured until #481 fixes it. Without a cluster, `TestCiphertextSurvivesReopeningADurableStore` decrypts across a close and reopen of the durable store. |
...
| Durability | **Verified, measured** | CI's restart probe (#483): `TestSecretManagerRestartSetup` runs alone just before `stop` and leaves a secret with a version. After `stop` and `up` in persistent mode, `TestSecretManagerAcrossRestart` reads the version back; brought up again with `--mode ephemeral`, it requires the secret to be NOT_FOUND (error code UNVERIFIED). The Secrets live in the workload namespace, which an ephemeral `up` keeps, so once the cluster answers and before `up` is ready an ephemeral run deletes every Secret Manager Secret of the instance not written by itself (each write carries the run's `cloudburrow.dev/epoch` label), and a persistent run deletes what an ephemeral run wrote. |
...
The storage server serves `notificationConfigs` and **detects the events itself** (#506).
Every event is written to an outbox in the same transaction as the mutation that caused it; a
dispatcher then publishes it to the Pub/Sub emulator after commit and deletes it once
published, retrying a failed publish (up to 20 attempts, so a topic that never exists cannot
hold the queue forever), so an event is never lost between the mutation and the publish. In an
instance the emulator is the cluster's Pub/Sub; `storage-server --pubsub-emulator host:port`
names one, and without one, creating a configuration is 501.

| Capability | Status | Notes |
|---|---|---|
| `notificationConfigs` insert / get / list / delete | **Verified** | `TestNotificationConfigsCRUDThroughTheSDK`, through the official Storage client; delete is 204 with no body. IDs are per bucket and never reused. At most 100 configurations per bucket and 10 custom attributes each; the refusal's code, 400, is UNVERIFIED. A configuration goes with its bucket. An identical configuration is not refused (UNVERIFIED). |
| Standard attributes and payload | **Verified** | `TestObjectUploadDeliversANotification`, uploaded with the Storage SDK and received with the Pub/Sub SDK: `eventType`, `bucketId`, `objectId`, `objectGeneration`, `payloadFormat`, `eventTime` and `notificationConfig`, with the Storage Object resource as the body. |
| Filters | **Verified** | `TestNotificationFiltersAreHonoured`: `object_name_prefix` and `event_types`, where an empty list means **all** types, as the API defines it. |
| `custom_attributes` and `payload_format: NONE` | **Verified in process** | `TestNotificationEventsAndFilters`: custom attributes are added and **cannot overwrite a standard attribute**; NONE sends attributes only. |
| Service-qualified topic form | **Verified** | The official client sends `//pubsub.googleapis.com/projects/{p}/topics/{t}` and parses it back; both spellings are accepted and the bare one stored. |
...
| Object upload, download, preview and delete | **Verified** | `TestConsoleStorageObjects` (#295), through the official storage client against the storage server: a console upload reads back through the SDK with the same bytes and CRC32C; a 10 MiB download is byte-identical; an HTML object previews as `text/plain` with a CSP and `nosniff`; an upload over the Settings limit (default 32 MiB) is refused with the limit named and leaves no object; a console delete removes the object. `TestDownloadIsStreamed` shows the download reaches the client before the provider's reader has finished. Previews are limited to 1 MiB, and only PNG, JPEG, GIF and WebP render as images. The upload limit lives in the running console and resets to the default on restart. On the storage server (#518), measured against a separate instance: `TestConsoleStorageObjects`, the other console tests, `TestReadyHooksRanAndCreatedABucket` (the #285 hook creates its bucket from the `STORAGE_EMULATOR_HOST` it is given) and `TestGcloudSetupConfiguration` pass, and `env` exports the one storage endpoint. The bucket list is scoped by project with no caveat, and a bucket's configuration shows its retention policy and lifecycle rule count. `TestAContainerListsBucketsFromPlainEnvFile` runs on Linux only, so in CI. |
...
| **Request-rate, latency and resource metrics** | **Partial** | **Cloud Tasks, Secret Manager, the Cloud Run v2 adapter and Cloud Storage**, the services CloudBurrow serves itself: every call is counted by service, method and canonical code, with a latency histogram, at `GET /metrics` on the loopback control port in the Prometheus text format (`TestCreateTaskCallsAreCounted`, which parses it with Prometheus's own parser). `/monitoring` charts request rate, error rate and p50/p95 latency per service. **Pub/Sub and the opt-in emulators are not measured**, since their traffic goes over a raw port-forward, and are labelled so, never charted at zero. Cloud Run workloads' own requests need Knative's queue-proxy metrics, which are off (#181). Storage (#513): each request is counted and recorded in `/admin/events` by API method (`storage.objects.insert`, `xml.PUT`), with its bucket, object, status and duration, and never its query string or body, so no upload_id, token, signature or secret (`TestStorageEventsRecorded`, `TestStorageEventsRedactUploadIDs`, `TestMetricsStorageIsMeasured`). The server also keeps its last 1000 calls at `GET /_cloudburrow/events?after=N`, for the CLI to scrape from the in-cluster server (#514). |
...
| Reset / seed / event inspection | Partial | `internal/admin`, served on the loopback-only control port and refused on service ports. **Reset covers Cloud Tasks, Cloud Storage (buckets, objects and notification configurations), Pub/Sub, Secret Manager and Cloud KMS**, narrowable with `service=` and `project=` (`TestAProjectResetClearsOnlyThatProject`). Storage buckets record their project (#510), so a project-scoped reset clears that project's buckets only (`TestAProjectResetIncludingStorage`). The storage server's reset clears its store directly: live, noncurrent and soft-deleted objects, soft-deleted buckets, upload sessions, rewrite tokens, notification configurations and undelivered events, multipart uploads, IAM policies and HMAC keys, with no API delete and so no soft delete or notification (`TestAStorageResetEmptiesEveryBucketOnBuiltin`, `TestAStorageProjectResetOnBuiltin`). Pub/Sub state in a project CloudBurrow has never seen survives a full reset. **Seed covers Cloud Tasks, Cloud Storage, Pub/Sub and Secret Manager** from one document validated whole before anything is created (`TestOneSeedDocumentIsReadBackByTheOfficialSDKs`, `TestAnInvalidSeedDocumentSeedsNothing`). Pub/Sub fields the emulator is not known to honour are refused by name. Bucket labels, location and storage class are seeded and kept (`TestStorageSeedAcceptsLabels`, #503). **Events record every API call on the ports CloudBurrow serves itself** — Cloud Tasks, Secret Manager (gRPC and JSON), the Cloud Run v2 adapter, and Cloud Storage, whose calls the CLI scrapes from the in-cluster server (#513, #514) — with method, resource, project, canonical status code and duration, and never a payload or query string (`TestSecretManagerCallsAreRecordedWithoutTheirPayloads`). **Pub/Sub and the opt-in emulators are not recorded**: their traffic goes over a raw port-forward to an upstream process that CloudBurrow does not intercept. |
