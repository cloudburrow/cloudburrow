...
| `buckets.patch` / `buckets.update` | **Partial** (fake-gcs-server) · **Verified** (builtin) | On the default fake-gcs-server backend `defaultEventBasedHold` is kept, `versioning` only in ephemeral mode (persistent mode refuses it with HTTP 500 "fs storage type does not support versioning yet"), and labels, storage class, CORS, website, retention policy and lifecycle rules are **accepted and silently discarded**, on create as on patch; a patch also resets each kept field it omits (#321, #374). It stays Partial until the builtin server replaces it (#519). On the builtin server a patch is a JSON merge patch that keeps what it omits, and every settable field is kept or refused by name (see *Builtin bucket fields* above): `TestBucketUpdateChangesMetadata`, `TestStorageBucketLabelsAndStorageClassRoundTrip`, `TestStorageBucketFieldsNeverSilentlyDropped`. |
...
| HMAC keys (`projects.hmacKeys` create, get, update, delete, list) and `projects.serviceAccount` | **Verified** (builtin) · **Unimplemented** (fake-gcs-server) | Builtin server (#505): a key's access ID is 61 characters and its secret 40 base64 characters, **returned once**, in the create response, and in no later response, log or event. At most 10 keys per service account; deleted ones do not count (the refusal's code, 400, is UNVERIFIED). A key is ACTIVE or INACTIVE; a delete while ACTIVE is 400, and a deleted key is DELETED, listed only with `showDeletedKeys`, and cannot change again. The service account email is not checked against any IAM service account, since none exist here. `serviceAccount` is `service-PROJECT_NUMBER@gs-project-accounts.iam.gserviceaccount.com`. The secret is kept in the server's store, for signing (#509), as Secret Manager keeps payloads. `TestStorageHMACKeyLifecycle`, `TestStorageHMACKeyLimit`. |
...
| Signed URLs | **Not verified** (fake-gcs-server) · **Verified, failing closed** (builtin) | On the default fake-gcs-server backend a signed URL is accepted on shape alone (a bogus `X-Goog-Signature` was served with 200, measured), so it cannot test signing correctness, until the builtin server replaces it (#519). The builtin server verifies signatures on the XML endpoints (#509): V4 `GOOG4-HMAC-SHA256` (and `AWS4-HMAC-SHA256` with `X-Amz-*`) against an ACTIVE HMAC key (#505), and V4 `GOOG4-RSA-SHA256` and V2 against certificates registered with `storage-server --signing-cert email=cert.pem` (`storage.signingCerts`). The canonical request is built as the docs define it, with the host accepted with or without its port, since the Go client signs the host name and the Python client `host:port`. `X-Goog-Expires` is at most 604800 (400 over it, UNVERIFIED code), and expiry is judged on the server's clock. A bad signature is **403 `SignatureDoesNotMatch`**; an unknown or inactive key, a service account with no registered certificate, or an expired URL is **403 `AccessDenied`**; never 200. `TestStorageSignedURLV4HMAC` (signed by hand, because the Go client signs V4 only with RSA), `TestStorageSignedURLV2RegisteredCert` (the Go client's V2 and V4 RSA), and, measured locally, Python's `generate_signed_url` v4 and v2 (`test_generate_signed_url_v4` runs in CI with #516). Signing a URL without a local key needs `iamcredentials` `signBlob`, which is not served. |
...
| Cloud Storage | **Verified** | `TestStateRestoresStorage`: buckets, objects with their bytes, content type and metadata, a 10 MiB object streamed through the archive, and notification configurations, restored with matching CRC32C. Objects stream into and out of the archive and are never held whole in memory, and a restored object whose CRC32C differs fails the load. Restoring goes through the bare backend, so it fires no notifications. Bucket labels, location and storage class are saved, but the backend discards them (see Seeding). **On the builtin server (#511)** the state is exported from its store directly (`builtinStorageSnapshotter`): every bucket with all its fields, soft-deleted buckets, every object version (live, noncurrent and soft-deleted) with its generation, metageneration, holds, retention and customTime, notification configurations and undelivered events, IAM policies, sessions and multipart uploads, and the bytes, streamed. A load verifies that every blob hashes to its content address and that every object version's CRC32C matches, before it replaces anything, in one transaction. **HMAC key secrets are excluded**, and the archive says so (`secretsExcluded`): a restored key keeps its metadata and cannot sign. `TestSnapshotRoundTripKeepsVersionsAndHolds`, `TestSnapshotExcludesHMACSecrets`, `TestSnapshotImportVerifiesBytes`, `TestBuiltinStorageSnapshotRoundTrip` (10 MiB); `TestStateRestoresVersionsAndHolds` runs against an instance once `up` serves the builtin backend (#519). |
...
The builtin server (`--storage-backend builtin`, opt-in until #519) serves
...
| Field | Meaning |
|---|---|
| `service` | `tasks`, `secretmanager`, `run` or `kms`, and `storage` on the builtin backend (#513); required |
...
| Pub/Sub, the opt-in emulators, and Cloud Storage on fake-gcs-server | **Refused** | 400. They are reached through a port-forward to the upstream emulator, so CloudBurrow never sees their requests and a rule would never apply. |
| Cloud Storage on the builtin server | **Verified in-process** | `service=storage` is accepted only where the server runs in the same process as the admin API (#513), as in the unit tests; `up` runs it in the cluster (#514), where a rule cannot reach it, so there a storage rule is still refused. `method` globs the API method (`storage.objects.*`, `xml.GET`), and `project` does not apply (a storage resource is `b/{bucket}/o/{object}`). A faulted call answers with **storage's own JSON or XML error body** and the status (a gRPC `code` maps to its HTTP status), and the fault is recorded. `TestFaultsInterposedStorage` and `TestStorageFaultsUseStorageErrorBodies`; the compat `TestFaultsStorage` asserts the refusal on fake-gcs-server today, and the builtin path once `up` serves it (#519). |
...
| **Request-rate, latency and resource metrics** | **Partial** | **Cloud Tasks, Secret Manager and the Cloud Run v2 adapter only**, the services CloudBurrow serves itself: every call is counted by service, method and canonical code, with a latency histogram, at `GET /metrics` on the loopback control port in the Prometheus text format (`TestCreateTaskCallsAreCounted`, which parses it with Prometheus's own parser). `/monitoring` charts request rate, error rate and p50/p95 latency per service. **Storage, Pub/Sub and the opt-in emulators are not measured**, since their traffic goes over a raw port-forward, and are labelled so, never charted at zero. Cloud Run workloads' own requests need Knative's queue-proxy metrics, which are off (#181). On the builtin Cloud Storage backend, storage is measured too (#513): each request is counted and recorded in `/admin/events` by API method (`storage.objects.insert`, `xml.PUT`), with its bucket, object, status and duration, and never its query string or body, so no upload_id, token, signature or secret (`TestStorageEventsRecorded`, `TestStorageEventsRedactUploadIDs`, `TestMetricsStorageIsMeasured`). The server also keeps its last 1000 calls at `GET /_cloudburrow/events?after=N`, for the CLI to scrape from the in-cluster server (#514). |
