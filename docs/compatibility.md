...
| `buckets.patch` / `buckets.update` | **Partial** | `TestBucketUpdateChangesMetadata`, through `Bucket.Update`: `versioning` is kept and re-read rather than trusted from the response. fake-gcs-server keeps only `versioning` and `defaultEventBasedHold`. It **accepts and silently discards** labels, storage class, CORS, website, retention policy and lifecycle rules, on create as well as on patch (#321). A patch also **resets** either of the two fields it keeps when the patch omits it, where Cloud Storage would leave it unchanged. The test pins both limitations. |
