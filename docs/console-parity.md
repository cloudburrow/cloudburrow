...
| Request Log (Operations) | Every API call CloudBurrow served: time, service, method, resource, canonical code, duration. Filters by service, code and project, carried in the URL. Live over `/api/stream?stream=requests` (#291). | **Not a Google Cloud console screen.** It has no GCP reference, so no parity is claimed: it is modelled on LocalCloud's request logs and LocalStack's App Inspector. Cloud Tasks, Secret Manager and Cloud Run are observable, because CloudBurrow serves them itself, and so is Cloud Storage on the builtin server (#518), whose calls are scraped with the bucket and object they were for. Storage on fake-gcs-server, Pub/Sub and the opt-in emulators are listed as *requests not observable (direct port-forward to upstream emulator)*, never as an empty list. No payload or query string is recorded, and the page copies named fields only. |
