...
func (a protoAllowlister) checkInternal(prefix string, msg protoreflect.Message) (err error) {
	...
	msg.Range(func(fd protoreflect.FieldDescriptor, v protoreflect.Value) bool {
		...
		// Modified by CloudBurrow: a map or repeated field of messages is not a
		// nested message, and v.Message() panics on it (a request with labels
		// crashed the server). It is checked as a field, below.
		if fd.Kind() == protoreflect.MessageKind && !fd.IsMap() && !fd.IsList() { // if we have a nested message
			if _, ok := a[path]; ok {
