...
func TestKMSIamPolicyIsStoredNotEnforced(t *testing.T) {
	...
	ring, err := c.CreateKeyRing(ctx, &kmspb.CreateKeyRingRequest{Parent: loc, KeyRingId: "iam"})
	if err != nil {
		t.Fatal(err)
	}
	...
	for kind, res := range map[string]string{"ring": ring.GetName(), "key": key.GetName()} {
		t.Run(kind, func(t *testing.T) {
			...
			if _, err := c.SetIamPolicy(ctx, &iampb.SetIamPolicyRequest{Resource: res, Policy: &iampb.Policy{Etag: empty.GetEtag()}}); status.Code(err) != codes.Aborted {
			...
			if status.Code(err) != codes.Unimplemented || !strings.Contains(status.Convert(err).Message(), "condition") {
	...
	// ResourceIAM, as client code uses it.
	...
	if _, err := c.GetIamPolicy(ctx, &iampb.GetIamPolicyRequest{Resource: loc + "/keyRings/absent"}); status.Code(err) != codes.NotFound {
	...
	if _, err := c.GetIamPolicy(ctx, &iampb.GetIamPolicyRequest{Resource: ring.GetName() + "/importJobs/j"}); status.Code(err) != codes.Unimplemented ||
